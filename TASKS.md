...
| Today | ❌ пустой белый экран | ❌ пусто | ❌ нет WaterStorage |
...
- [ ] Модель данных: `WaterRecord` (id, amount, date), `UserProfile` (name, age, weight, goal)
- [ ] Сервис хранения: `WaterStorage` (CRUD для записей воды, UserDefaults или Core Data)
- [ ] Логика расчёта нормы воды по весу и активности
- [ ] Сохранение флага завершения онбординга в UserDefaults

### Приоритет 2 — Today (главный экран)
- [ ] UI: кольцо прогресса (выпито / норма)
- [ ] UI: пикер количества воды (мл)
- [ ] UI: кнопки быстрого добавления (100мл, 200мл, 300мл)
- [ ] UI: кнопка «Добавить»
- [ ] Логика: добавление записи через WaterStorage
- [ ] Логика: обновление кольца прогресса при добавлении
